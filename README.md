...
Pi stores [user-installed packages](https://pi.dev/docs/latest/packages) below that configuration directory, in `npm/` and `git/`.  For a reasonable approximation of XDG-ish separation, keep configuration in `$XDG_CONFIG_HOME` and bulky package payloads in `$XDG_DATA_HOME` while preserving Pi's expected layout.  Use relative symlinks from your chosen configuration directory, like this:
